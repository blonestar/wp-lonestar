...
- Vite/build constants in `inc/core/vite.php` now have `LONESTAR_`-prefixed primary names (`LONESTAR_DIST_DEF`, `LONESTAR_DIST_URI`, `LONESTAR_DIST_PATH`, `LONESTAR_JS_DEPENDENCY`, `LONESTAR_JS_LOAD_IN_FOOTER`, `LONESTAR_VITE_SERVER`, `LONESTAR_VITE_ENTRY_POINT`); the legacy unprefixed constants (`DIST_DEF`, `DIST_URI`, `DIST_PATH`, `JS_DEPENDENCY`, `JS_LOAD_IN_FOOTER`, `VITE_SERVER`, `VITE_ENTRY_POINT`) are still defined from the new ones for backward compatibility, and a legacy value set beforehand (e.g. in `wp-config.php`) is still honored. `lonestar_is_vite_dev_mode()` now also honors `LONESTAR_VITE_DEVELOPMENT` ahead of the legacy `IS_VITE_DEVELOPMENT` constant. Internal theme code (`vite.php`, `blocks-acf-enqueue.php`, `functions.php`) now reads the `LONESTAR_*` names exclusively. The `vite.php` header comment no longer mentions Tailwind (not part of this framework); it now describes Vite asset integration. See `docs/developer-guide.md` "Deprecated compatibility aliases".
- Production Vite asset handles are now prefixed to avoid colliding with plugins: the main script handle is `lonestar-main` (was `main`) and built CSS handles are `lonestar-{filename}` (was the bare filename, e.g. `main`). Backward-compatible dependency-only alias handles (`main`, `{filename}`) are re-registered automatically when nothing else already owns that handle, so a child theme/plugin listing `'main'` as a script/style dependency keeps working unchanged. Dev-mode handles (`lonestar-vite-client`, `lonestar-vite-entry`) were already prefixed.
- Added `lonestar_write_log()`, `lonestar_printr()`, `lonestar_shortcode_reserved()`, and `lonestar_shortcode_year()` as the primary names for the theme's debug helpers and `[R]`/`[Y]`/`[year]` shortcode callbacks. `write_log()`, `printr()`, `theme_shortcode_reserved()`, and `theme_shortcode_year()` remain as thin `function_exists()`-guarded compatibility wrappers (no deprecation notice, to avoid log noise); the shortcode tags themselves (`[R]`, `[Y]`, `[year]`) are unchanged. `write_log()` also now checks `defined('WP_DEBUG') && WP_DEBUG` instead of `true === WP_DEBUG`.
- Removed unreachable dead code after the `return $links;` statement in `modules_get_module_admin_links()` (`inc/core/modules_catalog.php`), previously marked "Deprecated unreachable compatibility code". `modules_get_module_php_files_for_scanning()` (its only caller) is no longer used internally; it remains public but is now documented as deprecated.
- `modules_handle_modules_admin_post()` (`inc/core/modules_admin.php`) now guards `$_SERVER['REQUEST_METHOD']` with `isset()` before reading it.
- Added a file-level docblock note in `inc/core/modules.php` documenting the ~80-function `modules_*` family as a legacy namespace scheduled to move to `lonestar_module_*` at the 1.0 release (not renamed in this phase — too large a surface for one pass).
...
- The Vite dev-server auto-probe (`lonestar_is_vite_dev_mode()`) now only runs its `wp_remote_get()` HTTP check when `wp_get_environment_type()` is `local` or `development`; other environment types (e.g. `staging`) must opt in explicitly via `IS_VITE_DEVELOPMENT` or `LONESTAR_VITE_DEV`. Added filter `lonestar_vite_dev_probe_enabled` to override the gate per site. See `docs/developer-guide.md`.
- `lonestar_remove_query_string_from_static_files()` (`inc/core/blocks-acf-enqueue.php`) no longer runs `filemtime()` on every enqueued theme asset on every request. It now only rewrites the `ver` query arg when the asset carries WordPress's default core-version `ver` (i.e. was enqueued without an explicit version — theme-registered block assets already pass explicit filemtime versions and are skipped). Path resolution now maps the template/stylesheet directory URI to `get_template_directory()`/`get_stylesheet_directory()` instead of `ABSPATH`, which was wrong whenever `WP_CONTENT_DIR` lives outside `ABSPATH` (e.g. Bedrock-style installs). Resolved mtimes are memoized per request. Added `lonestar_theme_asset_url_to_path()`; `lonestar_asset_url_to_path()` (ABSPATH-based) is kept only for backward compatibility and is no longer used internally.
- Consolidated block discovery caching: the separate `lonestar_acf_blocks_to_load_v3`, `lonestar_native_blocks_to_load_v2`, `lonestar_php_only_blocks_to_load_v1`, `lonestar_blocks_to_scan_v3_{ns}`, and `lonestar_block_asset_map_v4_{ns}` transients are replaced by a single request-memoized + transient-backed block runtime index (`lonestar_get_block_runtime_index()`, fixed transient key `lonestar_block_runtime_v1`, payload carries its own `cache_namespace` so a namespace change on deploy is detected and rebuilt without needing a new key). Each block.json is now decoded once per request/cache build and reused by the ACF/native/PHP-only register functions and the JS/CSS asset map, instead of four independent filesystem scans + JSON decodes. `lonestar_get_cached_block_directories()` and `lonestar_get_cached_block_asset_registration_map()` are kept as backward-compatible delegates. `lonestar_flush_block_discovery_caches()` deletes the new key and still deletes all legacy keys for cleanup on upgrade. Evaluated using core's `wp_register_block_metadata_collection()` (WP 6.7+/`wp-includes/blocks.php`) to avoid a second metadata read inside `register_block_type_from_metadata()`; not adopted, since it requires a static build-time manifest per base path and block roots here are dynamic (parent theme, active child theme, and any enabled module's `blocks/{acf,native,php-only}` folder, toggled at runtime) — the consolidated runtime index already avoids the repeat filesystem/JSON work without a build-step dependency.
- Fixed a module catalog i18n cache bug: `modules_get_module_catalog()` previously cached module labels/descriptions/admin-link labels already translated via `__()`/`translate()`, so an admin viewing wp-admin in a different locale than whichever request last rebuilt the transient saw the wrong language. The catalog now caches untranslated source values plus their textdomain, and translates on every read (`modules_localize_module_catalog()`), after both cache hits and fresh builds. Bumped the catalog transient schema from `v4` to `v5` (new key format; old `v4` entries are simply orphaned and expire via their existing TTL). Availability `status` messages were already recomputed fresh on every read via `modules_refresh_module_catalog_availability()` and needed no change.
- `modules_get_module_source_fingerprint()` no longer globs every module directory and calls `filemtime()` per entry on every request just to build the catalog cache key. In the `production` environment type it now hashes only the modules root directory mtimes (which change on add/remove/rename of a top-level module entry) plus the theme version. Non-production environment types keep the full per-entry fingerprint. Added filter `lonestar_module_fingerprint_mode` (`'full'|'fast'`).
