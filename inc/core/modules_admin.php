...
function modules_get_content_type_argument_display($args, $key)
{
    ...
    $value = $args[$key];
    if (is_bool($value)) {
        return $value ? __('Yes', 'lonestar') : __('No', 'lonestar');
    }
    if (is_array($value)) {
        return empty($value) ? __('Declared empty', 'lonestar') : wp_json_encode($value);
    }
    if (null === $value) {
        return __('Declared null', 'lonestar');
    }
    if ('' === $value) {
        return __('Declared empty', 'lonestar');
    }
    return is_scalar($value) ? (string) $value : __('Unsupported value', 'lonestar');
}
...
function modules_render_content_type_table($entries, $entity_type)
{
    ...
    if (!is_array($entries) || empty($entries)) {
        echo '<p>' . esc_html($is_taxonomy ? __('No taxonomies were discovered.', 'lonestar') : __('No post types were discovered.', 'lonestar')) . '</p>';
        return;
    }
    echo '<table class="widefat striped" style="max-width:1200px;"><thead><tr>';
    echo '<th>' . esc_html($is_taxonomy ? __('Taxonomy', 'lonestar') : __('Post Type', 'lonestar')) . '</th><th>' . esc_html__('Status', 'lonestar') . '</th><th>' . esc_html__('Source', 'lonestar') . '</th><th>' . esc_html__('Configuration', 'lonestar') . '</th>';
    echo '</tr></thead><tbody>';
    foreach ($entries as $entry) {
        ...
        if (!empty($entry['filtered_out'])) $status[] = __('Filtered out', 'lonestar'); elseif (!empty($entry['filtered'])) $status[] = __('Filter modified', 'lonestar');
        $status[] = !empty($entry['registered']) ? __('Slug exists in WordPress', 'lonestar') : __('Slug does not exist in WordPress', 'lonestar');
        $summary = array();
        if ($is_taxonomy) {
        ...
        if (!empty($entry['effective']) && !empty($entry['registered'])) {
            if (!$is_taxonomy && function_exists('get_post_type_object')) {
                ...
                if ($can_edit_posts) echo '<br /><a href="' . esc_url(add_query_arg('post_type', $slug, admin_url('edit.php'))) . '">' . esc_html__('View posts', 'lonestar') . '</a>';
                if ($can_edit_posts && function_exists('get_post_type_archive_link')) { $url = get_post_type_archive_link($slug); if (is_string($url) && '' !== $url) echo ' | <a href="' . esc_url($url) . '">' . esc_html__('View archive', 'lonestar') . '</a>'; }
            }
            if ($is_taxonomy && function_exists('get_taxonomy')) {
                ...
                if ($object && !empty($object->show_ui) && isset($object->cap->manage_terms) && current_user_can($object->cap->manage_terms)) {
                    ...
                    echo '<br /><a href="' . esc_url(add_query_arg($query, admin_url('edit-tags.php'))) . '">' . esc_html__('View terms', 'lonestar') . '</a>';
                }
            }
        }
