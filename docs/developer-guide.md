...
1. `IS_VITE_DEVELOPMENT` constant, if defined — explicit override, always wins.
...
4. Otherwise, an automatic HTTP probe against `VITE_SERVER` (`GET {VITE_SERVER}/@vite/client`, cached per-namespace transient for one minute) — but only when `wp_get_environment_type()` is `local` or `development`. Other environment types (e.g. `staging`) never auto-probe; use `IS_VITE_DEVELOPMENT` or `LONESTAR_VITE_DEV` there instead.
