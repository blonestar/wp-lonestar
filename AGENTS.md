...
- CI covers Node 22/24 contract/build/audit checks, the CSS pipeline contract, PHP 8.2/8.4/8.5 lint, blocking PHPCS/PHPStan quality gates, and a blocking `wp-env frontend/admin smoke test` (`@wordpress/env` frontend/admin and block-registration check). There is no PHPUnit or browser test suite.
